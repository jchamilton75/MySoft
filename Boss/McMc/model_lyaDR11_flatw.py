...
# no prior on h
h = pymc.Uniform('h',0.,2.,value=0.7)
...
def thelogproba(h=h,om=om,w=w,ob=ob):
...
#clone just used to be called from outside for debugging purpose
...
def logp(value=0,h=h,om=om,w=w,ob=ob):
    return(thelogproba(h=h.flatten(),om=om.flatten(),w=w.flatten(),ob=ob.flatten()))
