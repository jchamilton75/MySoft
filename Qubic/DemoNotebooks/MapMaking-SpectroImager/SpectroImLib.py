...
def create_input_sky(parameters):
  ...
  spectra_dust = [np.zeros(len(ell)), 
                  ...
                  np.zeros(len(ell))]
  dust = np.array(hp.synfast(spectra_dust, parameters['nside'], new=True, pixwin=True)).T
...
def create_acquisition_operator_TOD(pointing, parameters):
  ...
  atod = QubicMultibandAcquisition(q, pointing, s, nus_edge_in, effective_duration=parameters['effective_duration'])
  ######################################################################################
...
def create_acquisition_operator_REC(pointing, parameters):
  ...
  arec = QubicMultibandAcquisition(q, pointing, s, nus_edge, effective_duration=parameters['effective_duration'])
  ######################################################################################
  return arec
...
def create_TOD(parameters, pointing, x0):
  ...
  TOD, maps_convolved_useless = atod.get_observation(x0)
