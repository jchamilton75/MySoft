...
instFI = QubicInstrument(filter_nu=150e9)
...
hp.gnomview(np.log10(sbidealFI/np.max(sbidealFI))*10, rot=[0,90], reso=5, 
	sub=(1,2,1), title='First Instrument', min=mini, max=0, unit='dB')
hp.gnomview(np.log10(sbidealTD/np.max(sbidealTD))*10, rot=[0,90], reso=5, 
	sub=(1,2,2), title='Technological Demonstrator', min=mini, max=0, unit='dB')
...
idet = 231
nside = 1024
sbidealFI_150 = getsb(hornsFI, 150., idet, dnu_nu=0.25, nsubnus=10, detector_integrate=4, nside=nside)
sbidealTD_150 = getsb(hornsTD, 150., idet, dnu_nu=0.25, nsubnus=10, detector_integrate=4, nside=nside)
