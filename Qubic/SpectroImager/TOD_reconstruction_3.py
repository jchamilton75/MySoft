...
def TOD(subnu_min,subnu_max,subdelta_nu,cmb,dust,sampling,scene,effective_duration,verbose=True):
	...
	if verbose:
		print('Nombre de bandes utilisées pour la construction : '+str(Nbsubbands)+'; sous fréquences centrales utilisées pour la reconstruction : '+str(sub_nus))
	...
	for i in range(Nbsubbands):
		...
		Y+=operator*pack*C*sub_deltas[i]*10**9*x0[i]

	noise=sub_acqs[0].get_noise()*np.sum(sub_deltas)*10**9
	Y+=noise
...
def convolved_true_maps(nu_min,nu_max,delta_nu,subdelta_nu,cmb,dust,verbose=True):
	...
	if verbose:
		print('Nombre de bandes utilisées pour la construction : '+str(Nbsubbands)+' ; sous fréquences centrales utilisées pour la reconstruction : '+str(sub_nus)+  ' ; fréquences centrales des bandes à reconstruire : '+str(nus))
...
def reconstruct(Y,nu_min,nu_max,delta_nu,subdelta_nu,sampling,scene,effective_duration, verbose=True):
	...
	if verbose:
		print('Nombre de fréquences utilisées pour la reconstruction : '+str(Nbsubfreq)+ ' ; Nombre de bandes reconstruites : '+str(len(bands))+' ; Résolution spectrale : '+str(delta_nu)+  ' ; bandes reconstruites : ' + str(bands))
