...
def Wigner3j(j1,j2,j3,m1,m2,m3, silent=True):
#======================================================================
...
#======================================================================
    ...
    if ( ( 2*j1 != floor(2*j1) ) | ( 2*j2 != floor(2*j2) ) | ( 2*j3 != floor(2*j3) ) | ( 2*m1 != floor(2*m1) ) | ( 2*m2 != floor(2*m2) ) | ( 2*m3 != floor(2*m3) ) ):
        if not silent: print 'All arguments must be integers or half-integers.'
        return -1

    # Additional check if the sum of the second row equals zero
    if ( m1+m2+m3 != 0 ):
        if not silent: print '3j-Symbol unphysical'
        return 0

    if ( j1 - m1 != floor ( j1 - m1 ) ):
        if not silent: print '2*j1 and 2*m1 must have the same parity'
        return 0
    
    if ( j2 - m2 != floor ( j2 - m2 ) ):
        if not silent: print '2*j2 and 2*m2 must have the same parity'
        return; 0

    if ( j3 - m3 != floor ( j3 - m3 ) ):
        if not silent: print '2*j3 and 2*m3 must have the same parity'
        return 0
    
    if ( j3 > j1 + j2)  | ( j3 < abs(j1 - j2) ):
        if not silent: print 'j3 is out of bounds.'
        return 0

    if abs(m1) > j1:
        if not silent: print 'm1 is out of bounds.'
        return 0

    if abs(m2) > j2:
        if not silent: print 'm2 is out of bounds.'
        return 0 

    if abs(m3) > j3:
        if not silent: print 'm3 is out of bounds.'
        return 0
