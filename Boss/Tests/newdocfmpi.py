...
from pairs import mr_wpairs_wp,mr_wpairs,mr_angpairs
import pyfits
...
def do_counting(comm,rank,bins,data1,data2,counter,weight1=None,weight2=None):
    ...
    elif counter == 'angular':
        wpairs = mr_angpairs.angular_wpairs(comm, data1, data2, w1=weight1, w2=weight2, ntrunc=1000)
    t2 = time.time()
...
def main(argv=None):
    ...
    counterChoices = ('euclidean','rppi','angular')
    ...
    comm = MPI.COMM_WORLD
    rank = comm.Get_rank()
    counts = do_counting(comm,rank,bins,data1,data2,opts.counter)
